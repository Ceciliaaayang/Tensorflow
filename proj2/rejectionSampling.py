...
def rejection_sampled_expectation(target, proposal, f=identity, num_samples=2000):
    ...
    acceptSample = [] #make a list of accept sample
    
    M = (proposal.max_pdf()/target.max_pdf()) #M*f(x)<=g(x), f(x) <= Mf, g(x) <= Mg, f(x)/g(x) <=Mf/Mg
    for i in range(num_samples):
        u = uniform.sample()
        p = proposal.sample()
        if u <= [(M * target.pdf(p))/(proposal.pdf(p))]:  #If u ≤ [Mf*(x) / g(x)], accept x (add it to the list of samples)  Else reject
            acceptSample.append(f(p))
    print('rejected sample:"{}"'.format (num_samples-len(acceptSample)))
    print(mean(acceptSample))
    return mean(acceptSample)
